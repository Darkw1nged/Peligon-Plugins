package net.peligon.Playtime.menus.inventoryItems;
...
public class PeligonPluginPlaceholders {
    ...
    public static ItemStack peligonAuthentication() {
        ...
        if (plugin.getServer().getPluginManager().getPlugin("PeligonAuthenticator") == null) {
    ...
    public static ItemStack peligonEconomy() {
        ...
        if (plugin.getServer().getPluginManager().getPlugin("PeligonEconomy") == null) {
            lore.add(Utils.chatColor("&8[Left-Click] &efor resource link."));
        } else {
            lore.add(Utils.chatColor("&8[Left-Click] &eto reload plugin."));
        }
