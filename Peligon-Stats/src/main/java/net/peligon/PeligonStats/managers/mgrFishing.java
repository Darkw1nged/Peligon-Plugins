...
public class mgrFishing {

    public static mgrFishing getInstance;
    public mgrFishing() {
        getInstance = this;
    }
    ...
    public boolean hasAccount(OfflinePlayer player) {
        ...
        String query = "SELECT 1 FROM plg_skill_Fishing WHERE uuid='" + uuid + "';";
        try {
    ...
    public void createAccount(OfflinePlayer player, int level, int experience) {
        ...
        String uuid = String.valueOf(player.getUniqueId());
        String query = "INSERT INTO plg_skill_Fishing values('" + uuid + "'," + level + "," + experience + ");";
        try {
    ...
    public void setLevel(OfflinePlayer player, int amount) {
        String uuid = String.valueOf(player.getUniqueId());
        String query = "UPDATE plg_skill_Fishing SET rank=" + amount + " WHERE uuid='" + uuid + "';";
        try {
            Statement statement = SQLite.connection.createStatement();
            statement.execute(query);
        } catch (SQLException e) {
    ...
    public void addLevel(OfflinePlayer player, int amount) {
        if (!hasAccount(player)) return;
        String uuid = String.valueOf(player.getUniqueId());
        String query = "UPDATE plg_skill_Fishing SET rank = (SELECT rank FROM plg_skill_Fishing WHERE uuid='" + uuid + "') +" + amount + " WHERE uuid= '" + uuid + "';";
        try {
            Statement statement = SQLite.connection.createStatement();
            statement.execute(query);
        } catch (SQLException e) {
    ...
    public void setExperience(OfflinePlayer player, int amount) {
        String uuid = String.valueOf(player.getUniqueId());
        String query = "UPDATE plg_skill_Fishing SET experience=" + amount + " WHERE uuid='" + uuid + "';";
        try {
            Statement statement = SQLite.connection.createStatement();
            statement.execute(query);
        } catch (SQLException e) {
    ...
    public void addExperience(OfflinePlayer player, int amount) {
        if (!hasAccount(player)) return;
        String uuid = String.valueOf(player.getUniqueId());
        String query = "UPDATE plg_skill_Fishing SET experience = (SELECT experience FROM plg_skill_Fishing WHERE uuid='" + uuid + "') +" + amount + " WHERE uuid= '" + uuid + "';";
        try {
            Statement statement = SQLite.connection.createStatement();
            statement.execute(query);
        } catch (SQLException e) {
    ...
    public Integer getLevel(OfflinePlayer player) {
        ...
        String uuid = String.valueOf(player.getUniqueId());
        String query = "SELECT * FROM plg_skill_Fishing WHERE uuid='" + uuid + "';";
        try {
